...
exports.registrarPagamento = (req, res) => {
    const { idContrato, idComp, dataPag } = req.body;

    if (!idContrato || !idComp || !dataPag) {
...
exports.historicoPagamentos = (req, res) => {
    ...
    db.query(sql, [idContrato], (err, results) => {
        ...
        if (results.length === 0) {
            return res.json({ message: "Nenhum pagamento registrado." });
        }

        //formatando datas para mes de ano
        const historicoFormatado = results.map(row => ({
            dataPag: moment(row.dataPag).format("MMMM [de] YYYY")
        }));

        res.json({ historico: historicoFormatado});
